...
import com.axone_io.ignition.git.utils.CommitMessageValidator;
...
public class CommitPopup extends JFrame {
    ...
    public CommitPopup(Object[][] data, Component parent) {
        ...
        // pre‑populate & validate on each keystroke ---------------------
        messageTextArea.setText(TEMPLATE);
        commitBtn.setEnabled(false);
        commitBtn.setToolTipText(EXAMPLE_TOOLTIP);
        updateCommitState();                // initial enable/tooltip check

        // ── validate on each keystroke ──────────────────────────────────
        messageTextArea.getDocument().addDocumentListener(new DocumentListener() {
            private void update() { updateCommitState(); }
            public void insertUpdate(DocumentEvent e){ update(); }
            public void removeUpdate(DocumentEvent e){ update(); }
            public void changedUpdate(DocumentEvent e){ update(); }
        });

        // ── validate on each checkbox toggle ────────────────────────────
        changesTable.getModel().addTableModelListener(e -> updateCommitState());
    ...
    // ------------------------------------------------------------------
    /** Enable commit only when message valid && at least one change selected. */
    private void updateCommitState() {
        boolean msgOk    = CommitMessageValidator.isValid(messageTextArea.getText());
        boolean changeOk = false;
        // Not efficient, temporary
        for (int i = 0; i < changesTable.getRowCount(); i++) {
            Object v = changesTable.getValueAt(i, 0);
            if (Boolean.TRUE.equals(v)) { changeOk = true; break; }
        }

        commitBtn.setEnabled(msgOk && changeOk);

        // context‑sensitive tooltip
        if (!msgOk)          commitBtn.setToolTipText(EXAMPLE_TOOLTIP);
        else if (!changeOk)  commitBtn.setToolTipText(NO_CHANGE_TOOLTIP);
        else                 commitBtn.setToolTipText(null);
    }
    ...
    private static final String EXAMPLE_TOOLTIP =
            "<html>Message must follow Conventional Commits!<br>"
            + "Example: <code>feat: added view to configure plots in runtime</code><br>"
            + "Example: <code>feat: added alarm pipeline to notify engineers when database down</code></html>";

    private static final String NO_CHANGE_TOOLTIP =
            "Select at least one modified resource to include in the commit";
